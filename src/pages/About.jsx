...
export default function About() {
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Story */}
      <section style={{ background: "#fff8f9" }} className="py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex justify-center">
              <div className="w-full max-w-sm aspect-[4/3] bg-white border border-[#f5b8c0] rounded-2xl flex items-center justify-center shadow-sm">
                <div className="text-center p-8">
                  <span className="text-8xl block mb-4">🍓</span>
                  <p className="font-display text-[#e8233a] text-2xl">since day one</p>
                  <p className="font-body text-[#6b7280] text-sm mt-1">made fresh, made with love</p>
                </div>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <h2 className="font-body font-semibold text-[#c41230] text-2xl md:text-3xl mb-5">how it all started</h2>
      ...
      {/* Values */}
      <section className="bg-white py-12 md:py-16 border-t border-[#fde8ea]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-body font-semibold text-[#c41230] text-2xl text-center mb-10">what we're about</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: <Leaf className="text-[#e8233a]" size={24} />, title: "Always Fresh", text: "We source the ripest, juiciest strawberries and prep everything daily. No shortcuts, no compromises.", emoji: "🍓" },
              { icon: <Heart className="text-[#e8233a]" size={24} />, title: "Made with Love", text: "Every cup is built by hand with care. We treat each order like it's for our own family.", emoji: "💕" },
              { icon: <Sparkles className="text-[#e8233a]" size={24} />, title: "A Little Bit Extra", text: "From Belgian chocolate to house-made matcha cream — we believe in going above and beyond.", emoji: "✨" },
            ].map((v, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                className="bg-white border border-[#f5b8c0] rounded-2xl p-6 text-center shadow-sm hover:scale-[1.02] transition-transform"
              >
                <div className="w-14 h-14 rounded-full bg-[#fde8ea] flex items-center justify-center mx-auto mb-4">{v.icon}</div>
                <h3 className="font-body font-bold text-[#1a1a1a] text-base mb-2">{v.emoji} {v.title}</h3>
      ...
      <section style={{ background: "#fff8f9" }} className="py-12 border-t border-[#fde8ea]">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <Link to="/location" className="bg-white border border-[#f5b8c0] text-[#1a1a1a] font-body font-bold px-7 py-3 rounded-full min-h-[44px] hover:bg-[#fde8ea] transition-colors">
              Find Us 🗺️
            </Link>
            <Link to="/menu" className="bg-[#e8233a] text-white font-body font-bold px-7 py-3 rounded-full min-h-[44px] hover:bg-[#c41230] transition-colors">
              View Menu 🍓
            </Link>
          </div>
        </div>
